...
public class Security_Config extends WebSecurityConfigurerAdapter{
	 ...
	 * */
		...
		http.exceptionHandling().accessDeniedHandler(accessDeniedHandler());//�޼��带ȣ���ؾ�����Ǽ� �����. 403����
	}
	...
	public AuthenticationSuccessHandler loginSuccessHandler() {
		return new LoginSuccessHandler();  //패키지 com.naver.securty에 존재하는 모든 클래스 파일에 @Service 없애주세요
	}
	
	/*
	 *1. UserDetailsService 인터페이스는 DB에서 유저 정보를 불러오는 loadUserByUsername()이 존재합니다.
	 *이를 구현하는 클래스는 DB에서 유저의 정보를 가져와서 uSERdETAILS 타입을 리턴해주는 작업을 합니다.
	 */
	
	@Bean
	public UserDetailsService customUserService() {
		return new CustomUserDetailsService();  //패키지 com.naver.security에 존재하는 모든 클래스 파일에 @Service를 없애주세요
	}
