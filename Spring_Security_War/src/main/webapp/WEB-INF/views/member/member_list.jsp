...
<title>회원관리 시스템 관리자모드(회원 목록 보기)</title>
 ...
 2. 검색 필드를 변경하면 검색어 입력창에 placeholder 나타나도록 합니다.
 	...
 	예로 아이디를 선택하면 placeholder로 "이름을 입력하세요"라고 나타납니다.
 	예로 아이디를 선택하면 placeholder로 "나이를 입력하세요"라고 나타납니다.
 	예로 아이디를 선택하면 placeholder로 "여 또는 남 입력하세요"라고 나타납니다.
 	
 3. 검색 버튼 클릭시 다음을 체크합니다.
 	1) 검색어를 입력하지 않은 경우 "검색어를 입력하세요"라고 대화상자가 나타나게 합니다.
 	2) 나이는 두 자리 숫자가 아닌 경우 "나이는 형식에 맞게 입력하세여(두자리 숫자)"라고 대화상자가 나탄게 합니다.
 ...
 --%>
 <script>
 	//search_field는 값을 계속 담아서 넘겨줘야함. 엑션에서 넘기고 있음 = index값임. 선택된 값이 없는 경우 -1으로 지정되어있음.
 	$(function(){
 		//검색 클릭 후 응답화면에는 검색시 필요한 필드가 선택되도록 합니다.
...
<body>
			...
			<table class="table table-striped">
				...
				<tbody>
					<c:forEach var ="m" items="${memberlist }">
						<tr>
							<td><a href="info?id=${m.id }">${m.id }</a></td>   <!-- 파라미터 id값으로 넘어오고 있음 -->
							<td>${m.name }</td>
	...
	<div class="center-block"> <!-- 페이징 처리 이전,숫자,다음  -->
		<ul class="pagination justify-content-center">  
			...
			<c:if test="${page > 1 }">
				<li class="page-item">
					<!-- <a href="memberList.net?page=${page-1 }" -->
