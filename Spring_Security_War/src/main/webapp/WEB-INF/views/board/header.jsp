...
<%@ taglib prefix="sec" uri="http://www.springframework.org/security/tags" %>  <!-- pom에서 추가한 tag !여기 추가됨!! -->
...
<%-- 기존 양식 : <c:if test="${empty id}"> --%>
<!-- isAnonymous() : 익명 사용자인 경우(로그인안한경우) 로그인 페이지로 이동하도록 합니다. -->
<sec:authorize access="isAnonymous()">   <!-- sec 시큐리티 약자 -->
   <script>
...
<nav class="navbar navbar-expand-sm right-block navbar-dark">
	...
	<!-- 
		...
		2. <sec:authentication property="principal" var="pinfo" />  태그라이브러리로 사용.
					인증된 사용자의 정보를 가져옵니다.
   	...
   	<sec:authorize access="isAuthenticated()">
         ...
         <li class="nav-item">
            <form action="${pageContext.request.contextPath }/member/logout" method="post"
                  ...
                  <input type="hidden" name="${_csrf.parameterName }" value="${_csrf.token }">  <!-- 포스트 방식이니까 토큰넣어줌 -->
            </form>
         </li>
